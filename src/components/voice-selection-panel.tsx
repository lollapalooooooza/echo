...
export function VoiceSelectionPanel({
  ...
  const sections = [
    {
      id: "preset",
      title: "Runway Defaults",
      subtitle: "Runway-provided live voice options, mapped to stable previews and speech playback.",
      ...
      {sections.map((section) => (
        <div key={section.id} className="space-y-3">
          ...
          {section.items.length > 0 ? (
            <div className={cn(section.id === "preset" && "max-h-[23rem] overflow-y-auto pr-1")}>
                          ...
                          onClick={async () => {
                            try {
                              await playVoicePreview({
                                previewKey: voice.key,
                                voiceId: voice.previewVoiceId,
                                text: previewText,
